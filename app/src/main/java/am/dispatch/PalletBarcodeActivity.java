...
import am.dispatch.Database.DatabaseHelper;
import am.dispatch.adapter.PalletAdpater;
import am.dispatch.model.Pallet;
import am.dispatch.model.RollInfo;
import am.dispatch.network.NetworkStateChecker;

public class PalletBarcodeActivity extends AppCompatActivity implements View.OnClickListener {
    EditText pallet_et, edit_barcode_no, roll_no_et, lot_no_et;
    Button btn_save;
...
//        rl_row_history = findViewById(R.id.rl_row_history);
        ...
        edit_barcode_no = findViewById(R.id.edit_barcode_no);
        lot_no_et = findViewById(R.id.edit_lot_no);
        roll_no_et = findViewById(R.id.edit_roll);
        ...
        pallet_et.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (s.toString().trim().length() == 0) {
                    edit_barcode_no.setVisibility(View.GONE);
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (et_pallet_count == 4) {
                    ...
                    edit_barcode_no.setVisibility(View.VISIBLE);
                    roll_no_et.setVisibility(View.VISIBLE);
                    lot_no_et.setVisibility(View.VISIBLE);
                    edit_barcode_no.clearFocus();
                    edit_barcode_no.requestFocus();
        ...
        lot_no_et.addTextChangedListener(new TextWatcher() {
        ...
        edit_barcode_no.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            @Override
        ...
        edit_barcode_no.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if (s.toString().trim().length() >= 1) {
                    btn_save.setVisibility(View.VISIBLE);
                    roll_no_et.setVisibility(View.GONE);
                    lot_no_et.setVisibility(View.GONE);
                } else {
                    btn_save.setVisibility(View.GONE);
                    roll_no_et.setVisibility(View.VISIBLE);
                    lot_no_et.setVisibility(View.VISIBLE);
...
//                loadNames();
    ...
    private void postDataUsingVolley(String rollnum, String pno, String usid, String device_id, String lot_no, String roll) {
        final ProgressDialog progressDialog = new ProgressDialog(this);
        progressDialog.setMessage("Saving ...");
        progressDialog.show();
        url = "https://art.artisticmilliners.com:8081/ords/art/bscan/dispatch/?bcode=" + rollnum + "&pno=" + pno + "&usid=" + usid + "&device_id=" + device_id + "&lotno=" + lot_no + "&rollno=" + roll;
        Log.e("URL342", url);
...
//                customdialog();
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                if (error.toString().contains("ServerError")) {
                    edit_barcode_no.setText("");
                    pallet_et.setText("");
                    Errorcustomdialog("Unexpected response", "Error");
    ...
    private void saveNameToLocalStorage(String pallet, String roll) {
        edit_barcode_no.setText("");
        db.addRollNumber(pallet, roll);
        RollInfo n = new RollInfo(pallet, roll);
        names.add(n);
        refreshList();
    }
    ...
    /*private void loadNames() {
        names.clear();
        Cursor cursor = db.getRollInfo();
        if (cursor.moveToFirst()) {
            do {
                @SuppressLint("Range") RollInfo name = new RollInfo(
...
//                        cursor.getInt(cursor.getColumnIndex(DatabaseHelper.COLUMN_STATUS))
        ...
        rollAdapter = new rolldataAdapter(this, R.layout.roll_info_list_text, names);
        rollNumberList.setAdapter(rollAdapter);
    }*/
    String roll_no;
    ...
    public void GET(String pallet_number) {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, "https://art.artisticmilliners.com:8081/ords/art/bscan/dispatch/?pno=" + pallet_number, null, new Response.Listener<JSONObject>() {
            @Override
    ...
    public void GETCount(String pallet_number) {
        ...
        RequestQueue requestQueue = Volley.newRequestQueue(this, hurlStack);
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, "https://art.artisticmilliners.com:8081/ords/art/bscan/dispatch_count/?pno=" + pallet_number, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                Log.e("response", response.toString());
                try {
    ...
    public void GETBARCODEvalidation(String barcode) {
        ...
        RequestQueue requestQueue = Volley.newRequestQueue(this, hurlStack);
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, "https://art.artisticmilliners.com:8081/ords/art/bscan/dispatch_validate/?bcode=" + barcode, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                Log.e("response", response.toString());
                rl_count.setVisibility(View.VISIBLE);
                try {
                    ...
                    for (int i = 0; i < obj.length(); i++) {
                        ...
                        if (barcode_text >= 1) {
                            ...
                            Log.e("Decode ", roll_no_et.getText().toString());
                            postDataUsingVolley(edit_barcode_no.getText().toString().trim(), pallet_et.getText().toString().trim(), "", androidID, "", "");
                        }
    ...
    public void GETDveiceID(String DEV_ID) {
        ...
        RequestQueue requestQueue = Volley.newRequestQueue(this, hurlStack);
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, "https://art.artisticmilliners.com:8081/ords/art/bscan/dispatch_device/?device_id=" + DEV_ID, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                Log.e("response", response.toString());
                try {
...
/*
   public void GET(String barcode) {
        ...
        RequestQueue requestQueue = Volley.newRequestQueue(this, hurlStack);
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, "https://art.artisticmilliners.com:8081/ords/art/bscan/dispatch/?bcode=" + barcode, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                Log.e("response", response.toString());
        ...
        try {
            ...
            listener = new ReadListener() {
                ...
                public void onRead(DecodeResult decodeResult) {
                    ...
                    edit_barcode_no.setText(decodeResult.getText());
//                    GET(decodeResult.getText());
                    if (roll_no_et.length() > 0 && lot_no_et.length() > 0) {
                        Log.e("Decode Result23", decodeResult.getText());
                        postDataUsingVolley("0", pallet_et.getText().toString().trim(), "", androidID, lot_no_et.getText().toString().trim(), roll_no_et.getText().toString().trim());

                    } else {
                        Log.e("Decode Result53", decodeResult.getText());
                        GETBARCODEvalidation(edit_barcode_no.getText().toString().trim());
//                        postDataUsingVolley(edit_barcode_no.getText().toString().trim(), pallet_et.getText().toString().trim(), "", androidID, "", "");
                    }
                }
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.btn_save:
                if (roll_no_et.length() > 0) {
                    Log.e("Decode Result455", roll_no_et.getText().toString());
                    postDataUsingVolley("0", pallet_et.getText().toString().trim(), "", androidID, lot_no_et.getText().toString().trim(), roll_no_et.getText().toString().trim());

                } else if (edit_barcode_no.length() > 0){
                    Log.e("Decode Resuwe5", edit_barcode_no.getText().toString());
                    GETBARCODEvalidation(edit_barcode_no.getText().toString().trim());
                }
                break;

        }
//        Log.e("Weight", edit_barcode_no.getText().toString().trim());
//        postDataUsingVolley(edit_barcode_no.getText().toString().trim(), pallet_et.getText().toString().trim(), "7426", edit_weight.getText().toString().trim(), shift);
    }
    ...
    private void DeleteDataUsingVolley(String rollnum, String pno, String device_id) {
        ...
        progressDialog.setMessage("Saving ...");
        progressDialog.show();
        //https://art.artisticmilliners.com:8081/ords/art/bscan/dispatch/?rollno=9&pno=123&device_id=
        url = "https://art.artisticmilliners.com:8081/ords/art/bscan/dispatch/?rollno=" + rollnum + "&pno=" + pno + "&device_id=" + device_id;
        Log.e("URL SAVE", url);
        HurlStack hurlStack = new HurlStack() {
            @Override
            protected HttpURLConnection createConnection(URL url) throws IOException {
